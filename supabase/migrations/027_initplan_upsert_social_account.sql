-- Fix H30: Wrap auth.uid() in subquery for InitPlan optimization
-- Migration 016 created this function AFTER migration 013 established the
-- (SELECT auth.uid()) pattern, but used bare auth.uid() instead.
-- Bare auth.uid() evaluates once per row; subquery evaluates once per query.
