...
export async function getPosts(filter: string, page: number, dateFilter?: string, opts?: QueryOptions) {
  return call<{ posts: unknown[]; total: number; page: number; totalPages: number }>('posts', { filter, page, dateFilter }, opts)
}

export async function getAccounts(opts?: QueryOptions) {
  return call<{ accounts: unknown[] }>('accounts', undefined, opts)
}

export async function getMedia(opts?: QueryOptions) {
  return call<{ media: unknown[] }>('media', undefined, opts)
}

export async function getCalendar(year: number, month: number, opts?: QueryOptions) {
  return call<{ posts: unknown[]; postsByDate: Record<string, unknown[]> }>('calendar', { year, month }, opts)
}

export async function getSettings(opts?: QueryOptions) {
  return call<{ profile: unknown }>('settings', undefined, opts)
}

export async function updateProfile(updates: Record<string, unknown>, opts?: QueryOptions) {
