...
exports.setNodeValidator = {
    ...
    validateNode(node, graphNode, ctx) {
        const issues = [];
        const nodeVersion = (0, string_utils_1.parseVersion)(node.version);
        ...
        if (mode !== undefined && (!isNonEmptyString(mode) || !SUPPORTED_MODES.has(mode))) {
            issues.push({
                ...
                message: `${nodeRef} uses unsupported Set node mode "${formatMode(mode)}". ` +
        ...
        for (const [index, assignment] of assignmentItems.entries()) {
            ...
            if (isNonEmptyString(assignment.name) && (0, validation_helpers_1.isCredentialFieldName)(assignment.name)) {
                issues.push({
