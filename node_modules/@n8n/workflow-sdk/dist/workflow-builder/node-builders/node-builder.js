...
exports.normalizeNodeConfig = normalizeNodeConfig;
...
function normalizeNodeConfig(config) {
    ...
    if (!creds)
        return { ...config };
    let normalizedCreds;
    const setNormalized = (key, next) => {
        normalizedCreds ??= { ...creds };
        normalizedCreds[key] = next;
    };
    for (const [key, value] of Object.entries(creds)) {
        if (typeof value === 'string' && (0, string_utils_1.isPlaceholderValue)(value)) {
            setNormalized(key, new NewCredentialImpl((0, string_utils_1.extractHint)(value)));
            continue;
        }
        ...
        const obj = value;
        if ('__newCredential' in obj)
            continue;
        if ('value' in obj && !('id' in obj)) {
            const v = String(obj.value);
            const name = (0, string_utils_1.isPlaceholderValue)(v) ? (0, string_utils_1.extractHint)(v) : v;
            setNormalized(key, new NewCredentialImpl(name));
            continue;
        }
        if ('id' in obj) {
            const idStr = typeof obj.id === 'string' ? obj.id : '';
            if ((0, string_utils_1.isPlaceholderValue)(idStr)) {
                const rawName = obj.name;
                const name = typeof rawName === 'string' && rawName.length > 0 && !(0, string_utils_1.isPlaceholderValue)(rawName)
                    ? rawName
                    : typeof rawName === 'string' && (0, string_utils_1.isPlaceholderValue)(rawName)
                        ? (0, string_utils_1.extractHint)(rawName)
                        : (0, string_utils_1.extractHint)(idStr);
                setNormalized(key, new NewCredentialImpl(name));
                continue;
            }
        }
...
function node(input) {
    (0, validation_helpers_1.assertPlainObject)(input, 'node', "a configuration object { type, version, config }. Example: node({ type: 'n8n-nodes-base.httpRequest', version: 4.2, config: { parameters: {} } })");
...
function ifElse(input) {
    (0, validation_helpers_1.assertPlainObject)(input, 'ifElse', 'a config object { version, config }');
    return node({
...
function merge(input) {
    (0, validation_helpers_1.assertPlainObject)(input, 'merge', 'a config object { version, config }');
    return node({
...
function switchCase(input) {
    (0, validation_helpers_1.assertPlainObject)(input, 'switchCase', 'a config object { version, config }');
    return node({
...
function trigger(input) {
    (0, validation_helpers_1.assertPlainObject)(input, 'trigger', "a configuration object { type, version, config }. Example: trigger({ type: 'n8n-nodes-base.webhook', version: 2, config: { parameters: {} } })");
    const versionStr = String(input.version);
    const config = input.output
        ? { ...input.config, output: input.output }
        : input.config;
