...
exports.tryMergeUxForkVariants = tryMergeUxForkVariants;
...
function generateResourceMapperTypeDeclaration(exported) {
    const prefix = exported ? 'export type' : 'type';
...
function tryMergeUxForkVariants(a, b) {
    if (!a || !b)
        return null;
    const aShow = a.show ?? {};
    const aHide = a.hide ?? {};
    ...
    const result = {};
    if (Object.keys(sharedShow).length > 0)
        result.show = sharedShow;
    if (forkKey === null) {
        if (Object.keys(sharedHide).length > 0)
            result.hide = sharedHide;
        return result;
    }
    const showSerialized = (showValues ?? []).map((s) => JSON.stringify(s));
    const remainingHide = (hideValues ?? []).filter((h) => !showSerialized.includes(JSON.stringify(h)));
    ...
    for (const prop of properties) {
        ...
        if (seenProps.has(normalizedProp.name)) {
            ...
            if (existingProp.displayOptions && normalizedProp.displayOptions) {
                const mergedDisplayOptions = tryMergeUxForkVariants(existingProp.displayOptions, normalizedProp.displayOptions);
...
function wrapMultipleValuesType(prop, typeStr) {
