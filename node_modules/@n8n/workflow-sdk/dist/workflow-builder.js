...
function assertNotOutputSelector(value, method) {
    if (!(0, node_builder_1.isOutputSelector)(value))
        return;
    const sourceName = value.node.name;
    throw new TypeError(`Cannot pass an OutputSelector to .${method}(). ` +
...
function createWorkflow(id, name, options) {
    if (typeof id !== 'string') {
        ...
        throw new TypeError('workflow() requires (id: string, name: string). ' +
    ...
    if (options !== undefined &&
        ...
        throw new TypeError('workflow() third argument is settings, not workflow structure. ' +
