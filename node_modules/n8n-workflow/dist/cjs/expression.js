...
(function (factory) {
    if (typeof module === "object" && typeof module.exports === "object") {
        ...
        define(["require", "exports", "@n8n/errors", "@n8n/expression-runtime", "luxon", "./errors", "./errors/expression-extension.error", "./errors/expression.error", "./expression-evaluator-proxy", "./expression-sandboxing", "./expressions/expression-helpers", "./logger-proxy", "./extensions", "./extensions/expression-extension", "./extensions/extended-functions"], factory);
    }
    ...
    function mapVmError(error) {
        ...
        if (error instanceof expression_runtime_1.TimeoutError) {
            const wrapped = new expression_error_1.ExpressionError('Expression timed out');
            wrapped.cause = error;
            return wrapped;
        }
        if (error instanceof expression_runtime_1.MemoryLimitError) {
            const wrapped = new expression_error_1.ExpressionError('Expression exceeded memory limit');
            wrapped.cause = error;
            return wrapped;
        }
        if (error instanceof expression_runtime_1.SecurityViolationError) {
            const wrapped = new expression_error_1.ExpressionError(error.message);
            wrapped.cause = error;
            return wrapped;
        }
        ...
        if (error instanceof Error && error.name === 'ExpressionExtensionError') {
            const reconstructed = new expression_extension_error_1.ExpressionExtensionError(error.message);
            Object.assign(reconstructed, error);
            return reconstructed;
        }
        if (error instanceof Error && error.name === 'ExpressionError') {
            const reconstructed = new expression_error_1.ExpressionError(error.message);
            Object.assign(reconstructed, error);
            return reconstructed;
        }
        if (isSyntaxError(error))
            return new expression_error_1.ExpressionError('invalid syntax');
    ...
    class Expression {
        timezone;
        static expressionEngine = 'legacy';
        static vmEvaluator;
        ...
        static async initExpressionEngine(options) {
            if (options.engine !== 'vm' || IS_FRONTEND)
                return;
            this.expressionEngine = options.engine;
            if (!this.vmEvaluator) {
                // Dynamic import to avoid loading expression-runtime in browser environments
                const { ExpressionEvaluator, IsolatedVmBridge } = await (__syncRequire ? Promise.resolve().then(() => __importStar(require('@n8n/expression-runtime'))) : new Promise((resolve_1, reject_1) => { require(['@n8n/expression-runtime'], resolve_1, reject_1); }).then(__importStar));
                this.vmEvaluator = new ExpressionEvaluator({
                    createBridge: () => new IsolatedVmBridge({
        ...
        async acquireIsolate() {
            if (Expression.vmEvaluator)
                await Expression.vmEvaluator.acquire(this);
        }
        async releaseIsolate() {
            if (Expression.vmEvaluator)
                await Expression.vmEvaluator.release(this);
        }
        /**
         * Dispose the VM evaluator and release resources.
         * Should be called during application shutdown or test teardown.
         */
        static async disposeExpressionEngine() {
            if (this.vmEvaluator) {
         ...
         */
        static getActiveImplementation() {
            if (this.shouldUseVm())
                return 'vm';
            return 'legacy';
        }
         ...
         */
        static setExpressionEngine(engine) {
        ...
        renderExpression(expression, data) {
            ...
            if (Expression.expressionEngine === 'vm' && !IS_FRONTEND) {
                if (!Expression.vmEvaluator) {
                    throw new errors_2.UnexpectedError('N8N_EXPRESSION_ENGINE=vm is enabled but VM evaluator is not initialized. Call Expression.initExpressionEngine() during application startup.');
                }
                try {
