...
import { DollarSignValidator, PrototypeSanitizer, ThisSanitizer, sanitizer, sanitizerName, } from './expression-sandboxing';
...
function mapVmError(error) {
    ...
    if (isSyntaxError(error))
        return new ExpressionError('invalid syntax');
...
export class Expression {
    ...
    renderExpression(expression, data) {
        ...
        if (Expression.expressionEngine === 'vm' && !IS_FRONTEND) {
            if (!Expression.vmEvaluator) {
                throw new UnexpectedError('N8N_EXPRESSION_ENGINE=vm is enabled but VM evaluator is not initialized. Call Expression.initExpressionEngine() during application startup.');
            }
            try {
